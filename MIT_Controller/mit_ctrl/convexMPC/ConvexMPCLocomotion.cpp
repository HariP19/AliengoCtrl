...
void ConvexMPCLocomotion::_SetupCommand(ControlFSMData<float> &data)
{
  if (data._quadruped->_robotType == RobotType::UNITREE_A1){
    ...
    _body_height = 0.45;
  } else if (data._quadruped->_robotType == RobotType::ALIENGO){
    _body_height = 0.45;
  } else 
  {
    assert(false);
  ...
  float x_vel_cmd = 0.0, y_vel_cmd = 0.0;
  float filter(0.1);
...
void ConvexMPCLocomotion::run(ControlFSMData<float> &data)
{
...
#ifdef DEBUG_MPC
  mpc_log.saveValue("\nINPUT-LOG-MPC\n");
  mpc_log.saveValue("_x_vel_des");
  mpc_log.saveValue(_x_vel_des);
  mpc_log.saveValue("_y_vel_des");
  mpc_log.saveValue(_y_vel_des);
  mpc_log.saveValue("_yaw_des");
  mpc_log.saveValue(_yaw_des);
  mpc_log.saveValue("Contact Estimate");
  mpc_log.saveVector(seResult.contactEstimate);
  mpc_log.saveValue("position");
  mpc_log.saveVector(seResult.position);
  mpc_log.saveValue("vBody");
  mpc_log.saveVector(seResult.vBody);
  mpc_log.saveValue("orientation");
  mpc_log.saveVector(seResult.orientation);
  mpc_log.saveValue("omegaBody");
  mpc_log.saveVector(seResult.omegaBody);
  mpc_log.saveValue("rBody");
  mpc_log.saveMatrix(seResult.rBody);
  mpc_log.saveValue("rpy");
  mpc_log.saveVector(seResult.rpy);
  mpc_log.saveValue("omegaWorld");
  mpc_log.saveVector(seResult.omegaWorld);
  mpc_log.saveValue("vWorld");
  mpc_log.saveVector(seResult.vWorld);
  mpc_log.saveValue("aBody");
  mpc_log.saveVector(seResult.aBody);
  mpc_log.saveValue("aWorld");
  mpc_log.saveVector(seResult.aWorld);
  ...
  for (int foot = 0; foot < 4; foot++)
  {
    ...
    #ifdef DEBUG_MPC
        mpc_log.saveValue("\nMPC OUTPUT_LOG\n");
        mpc_log.saveValue("Foot: " + std::to_string(foot));
        mpc_log.saveValue("pDesLeg");
        mpc_log.saveVector(data._legController->commands[foot].pDes);
        mpc_log.saveValue("vDesLeg");
        mpc_log.saveVector(data._legController->commands[foot].vDes);
        mpc_log.saveValue("f_ff");
        mpc_log.saveVector(data._legController->commands[foot].forceFeedForward);
